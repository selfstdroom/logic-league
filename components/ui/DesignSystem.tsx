...
export function PageShell({ children, className = "max-w-6xl" }: { children: ReactNode; className?: string }) {
  return <main className={`mx-auto px-4 py-5 sm:px-6 sm:py-7 lg:py-10 ${className}`}>{children}</main>;
}
...
export function HeroPanel({ eyebrow, title, children, actions, className = "" }: { eyebrow: string; title: ReactNode; children?: ReactNode; actions?: ReactNode; className?: string }) {
  return (
    <section className={`relative overflow-hidden rounded-[1.75rem] border border-amber-300/20 bg-[radial-gradient(circle_at_top_right,rgba(215,180,106,0.18),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.07),rgba(8,13,26,0.78))] p-5 shadow-2xl sm:p-7 lg:p-8 ${className}`}>
      ...
      <div className="relative">
        ...
        <h1 className="mt-3 max-w-4xl text-3xl font-black leading-tight text-white sm:text-4xl lg:text-5xl">{title}</h1>
        {children ? <div className="mt-4 max-w-3xl text-sm leading-7 text-league-silver sm:text-base">{children}</div> : null}
