...
export default async function WeeklyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const submissionBlockReason = !user ? "visitor" : !currentProfile?.qualified ? "unqualified" : "closed";
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex flex-col items-start gap-3 sm:items-end">
                    ...
                    <WeeklyVoteButton
                      topicId={topic.id}
                      answerId={answer.id}
                      canVote={canVote && answer.user_id !== user?.id && !votedAnswerIds.has(answer.id)}
                      voted={votedAnswerIds.has(answer.id)}
                      blockReason={!user ? "visitor" : !currentProfile?.qualified ? "unqualified" : remainingVotes <= 0 ? "no_votes" : answer.user_id === user.id ? "own_answer" : null}
                    />
                  </div>
                </div>
