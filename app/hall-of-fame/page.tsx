...
export default async function HallOfFamePage() {
  ...
  const { data } = await admin
    .from("hall_of_fame")
    .select("id, final_score, created_at, topics(title, publish_at), profiles:profiles!hall_of_fame_winner_user_id_fkey(username, display_name, rank), topic_answers:topic_answers!hall_of_fame_winner_answer_id_fkey(content)")
            ...
            return (
              <Link key={row.id} href={`/hall-of-fame/${row.id}`} className="block">
                <Card className="h-full border-amber-300/20 bg-[linear-gradient(145deg,rgba(215,180,106,0.12),rgba(255,255,255,0.04))] transition hover:-translate-y-1 hover:border-amber-300/45">
                  <p className="text-xs font-black uppercase tracking-[0.26em] text-league-gold">{formatDateTime(topic?.publish_at ?? row.created_at)}</p>
